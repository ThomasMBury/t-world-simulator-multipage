function [output] = model_TWorld(t,y,flag_ode, cellType, ICaL_Multiplier, ...
    ...
    INaK_Multiplier, INab_Multiplier, ICab_Multiplier, IpCa_Multiplier, IClCa_Multiplier, IClb_Multiplier, Jrel_Multiplier,Jup_Multiplier, ions_na_o,ions_ca_o,ions_k_o,ICaL_fractionSS,INaCa_fractionSS, PKA_P, Whole_cell_PP1, runSignallingPathway, constantsSig, sex, stimAmp, stimDur, vcParameters, apClamp, extraParams)


%% parameter sex can be used for sex-specific switches
CMDN_Multiplier = 1;
if strcmp(sex, 'M')
    ...
    IpCa_Multiplier = IpCa_Multiplier * 0.8; % Differs to ToR-ORd, may need to revisit this
...
%% PKA signalling - simulating or using direct phosphorylation values
if runSignallingPathway == 1
    ...
    %     ydot_signalling = zeros(57,1);

end

%% Naming variables for better readability
membrane_v = y(1);
...
ions_cl_o = 150;  % Extracellular Cl  [mM]
ions_mg_i = 0.5;  % Intracellular Mg  [mM]

% Nernst Potentials
ena_junc = (1/FoRT)*log(ions_na_o/ions_na_junc);     % [mV]
ena_sl = (1/FoRT)*log(ions_na_o/ions_na_sl);       % [mV]
ek = (1/FoRT)*log(ions_k_o/ions_k_i);	        % [mV]
eca_junc = (1/FoRT/2)*log(ions_ca_o/ions_ca_junc);   % [mV]
eca_sl = (1/FoRT/2)*log(ions_ca_o/ions_ca_sl);     % [mV]
ecl = (1/FoRT)*log(ions_cl_i/ions_cl_o);            % [mV]
...
Bmax_CaM = 24e-3 * CMDN_Multiplier;       % [mM] % CaM buffering
...
Bmax_SR = 17.85854e-3;     % [mM]  19e-3
koff_sr = 60e-3;        % [1/ms]
kon_sr = 100;           % [1/mM/ms]
Bmax_SLlowsl = 33.923e-3*Vmyo/Vsl;        % [mM]    % SL buffering
...
koff_slh = 30e-3;       % [1/ms]
kon_slh = 100;          % [1/mM/ms]
Bmax_Csqn = 136.55214e-3*Vmyo/Vsr;            % [mM]   % Csqn buffering
koff_csqn = 65;         % [1/ms]
...
% ina_hp, ina_jp, ina_h_PKA, ina_j_PKA, ina_h_dualP, ina_j_dualP
% [I_NaFast_junc, I_NaFast_sl, d_ina_m, d_ina_h, d_ina_j] = getINaFast(membrane_v, ina_m, ina_h, ina_j, ena_junc, ena_sl, Fjunc, Fsl, INa_Multiplier);
% if fINa_PKA > 0
...
% else
%     [I_NaFast_junc, I_NaFast_sl, d_ina_m, d_ina_h, d_ina_hp, d_ina_j, d_ina_jp, d_ina_h_PKA, d_ina_j_PKA, d_ina_h_dualP, d_ina_j_dualP] = getINa_Grandi_noPKA(membrane_v, ina_m, ina_h, ina_hp, ina_j, ina_jp, fINap, ena_junc, ena_sl, INa_Multiplier,fINa_PKA, Fjunc,ina_h_PKA, ina_j_PKA, ina_h_dualP, ina_j_dualP);
% end
...
GNaB = 2* 0.297e-3 * INab_Multiplier;    % [mS/uF]
...
KmNaip_PKA = 8.4615;
KmKo = 1.5;         % [mM]
fnak = 0.75 + (0.00375- ((140 - ions_na_o)/50) * 0.001)*membrane_v ; % Varying the slope mainly based on https://rupress.org/jgp/article-pdf/94/3/539/1814046/539.pdf
...
[I_kr, d_ikr_c0, d_ikr_c1, d_ikr_c2, d_ikr_o, d_ikr_i ] = getIKr_ToRORd_MM(membrane_v,ikr_c0,ikr_c1, ikr_c2, ikr_o, ikr_i,...
    ions_k_o, ek, cellType, IKr_Multiplier);
...
GClB = IClb_Multiplier * 0.00241;        % [mS/uF]
KdClCa = 100e-3;    % [mM]

I_ClCa_junc = 0.5 * Fjunc*GClCa/(1+KdClCa/ions_ca_junc)*(membrane_v-ecl);
I_ClCa_sl = 0.5 * Fsl*GClCa/(1+KdClCa/ions_ca_sl)*(membrane_v-ecl);
...
if (fICaL_PKA > 0)
    [I_Ca_junc,I_CaNa_junc,ICaK_junc,I_Ca_sl,I_CaNa_sl,ICaK_sl,d_ical_d, d_ical_ff,d_ical_fs,d_ical_fcaf,d_ical_fcas,d_ical_jca,d_ical_nca,d_ical_nca_i,...
        d_ical_ffp, d_ical_fcafp, d_ical_d_P,d_ical_ff_P,d_ical_fs_P,d_ical_fcaf_P,d_ical_fcas_P,d_ical_fBPf,d_ical_fcaBPf, d_ical_pureCDI_junc, d_ical_pureCDI_sl, PhiCaL_ss, PhiCaL_i, gammaCaoMyo, gammaCaiMyo] = getICaL_ORd2011_jt(membrane_v, ical_d,ical_ff,ical_fs,ical_fcaf,ical_fcas,ical_jca,ical_nca,ical_nca_i,ical_ffp,ical_fcafp,...
        ical_d_P,ical_ff_P,ical_fs_P,ical_fcaf_P,ical_fcas_P,ical_fBPf,ical_fcaBPf, ...
        ical_pureCDI_junc, ical_pureCDI_sl, fICaLp, fICaL_PKA, ions_ca_i, ions_ca_sl, ions_ca_junc, ions_ca_o, ions_na_i, ions_na_sl, ions_na_o, ions_k_i, ions_k_o, ions_cl_i, ions_cl_o, cellType, ICaL_fractionSS, ICaL_Multiplier, extraParams );
else
    [I_Ca_junc,I_CaNa_junc,ICaK_junc,I_Ca_sl,I_CaNa_sl,ICaK_sl,d_ical_d, d_ical_ff,d_ical_fs,d_ical_fcaf,d_ical_fcas,d_ical_jca,d_ical_nca,d_ical_nca_i,...
        d_ical_ffp, d_ical_fcafp, d_ical_d_P,d_ical_ff_P,d_ical_fs_P,d_ical_fcaf_P,d_ical_fcas_P,d_ical_fBPf,d_ical_fcaBPf, d_ical_pureCDI_junc, d_ical_pureCDI_sl, PhiCaL_ss, PhiCaL_i, gammaCaoMyo, gammaCaiMyo] = getICaL_ORd2011_noPKA_jt(membrane_v, ical_d,ical_ff,ical_fs,ical_fcaf,ical_fcas,ical_jca,ical_nca,ical_nca_i,ical_ffp,ical_fcafp,...
        ical_d_P,ical_ff_P,ical_fs_P,ical_fcaf_P,ical_fcas_P,ical_fBPf,ical_fcaBPf, ...
        ical_pureCDI_junc, ical_pureCDI_sl, fICaLp, fICaL_PKA, ions_ca_i, ions_ca_sl, ions_ca_junc, ions_ca_o, ions_na_i, ions_na_sl, ions_na_o, ions_k_i, ions_k_o, ions_cl_i, ions_cl_o, cellType, ICaL_fractionSS, ICaL_Multiplier, extraParams );
end
...
% ions_na_sl = 8;
% ions_na_i = 8;
[ I_ncx_sl, I_ncx_junc] = getINaCa_ORd2011(membrane_v,F,R,T, ions_na_junc, ions_na_sl, ions_na_i, ions_na_o, ions_ca_junc, ions_ca_sl, ions_ca_i, ions_ca_o, cellType, INaCa_Multiplier, INaCa_fractionSS);
...
d_buffers_NaBj = kon_na*ions_na_junc*(Bmax_Naj-buffers_NaBj)-koff_na*buffers_NaBj;        % NaBj      [mM/ms]
d_buffers_NaBsl = kon_na*ions_na_sl*(Bmax_Nasl-buffers_NaBsl)-koff_na*buffers_NaBsl;       % NaBsl     [mM/ms]

d_buffers_TnClow = Bmax_TnClow * d_contraction_Ca_TRPN; % myofilament - based on LAND contraction model
d_buffers_TnCHc = kon_tnchca*ions_ca_i*(Bmax_TnChigh-buffers_TnCHc-buffers_TnCHm)-koff_tnchca*buffers_TnCHc; % TnCHc     [mM/ms]
d_buffers_TnCHm = kon_tnchmg*ions_mg_i*(Bmax_TnChigh-buffers_TnCHc-buffers_TnCHm)-koff_tnchmg*buffers_TnCHm;   % TnCHm     [mM/ms]
d_buffers_CaM = kon_cam*ions_ca_i*(Bmax_CaM-buffers_CaM)-koff_cam*buffers_CaM;                 % CaM       [mM/ms]
d_buffers_Myosin_ca = kon_myoca*ions_ca_i*(Bmax_myosin-buffers_Myosin_ca-buffers_Myosin_mg)-koff_myoca*buffers_Myosin_ca;    % Myosin_ca [mM/ms]
d_buffers_Myosin_mg = kon_myomg*ions_mg_i*(Bmax_myosin-buffers_Myosin_ca-buffers_Myosin_mg)-koff_myomg*buffers_Myosin_mg;      % Myosin_mg [mM/ms]
d_buffers_SRB = kon_sr*ions_ca_i*(Bmax_SR-buffers_SRB)-koff_sr*buffers_SRB;                    % SRB       [mM/ms]

J_CaB_cytosol = d_buffers_TnClow + d_buffers_TnCHc + d_buffers_CaM + d_buffers_Myosin_ca + d_buffers_SRB;

% Junctional and SL Ca Buffers
d_buffers_SLLj = kon_sll*ions_ca_junc*(Bmax_SLlowj-buffers_SLLj)-koff_sll*buffers_SLLj;       % SLLj      [mM/ms]
d_buffers_SLLsl = kon_sll*ions_ca_sl*(Bmax_SLlowsl-buffers_SLLsl)-koff_sll*buffers_SLLsl;      % SLLsl     [mM/ms]
d_buffers_SLHj = kon_slh*ions_ca_junc*(Bmax_SLhighj-buffers_SLHj)-koff_slh*buffers_SLHj;      % SLHj      [mM/ms]
d_buffers_SLHsl = kon_slh*ions_ca_sl*(Bmax_SLhighsl-buffers_SLHsl)-koff_slh*buffers_SLHsl;     % SLHsl     [mM/ms]
...
if flag_ode==1
    ...
    V = membrane_v;
    output = [I_NaFast, I_NaL, I_to, I_Ca, I_kr, I_ks, I_k1, I_ncx_junc, I_ncx_sl, I_nak, I_Kb, I_Nabk, I_cabk, I_pca, J_SRCarel, 0, 0,...
        J_SRleak, 0, 0, I_ClCa, I_ClCa_junc, I_ClCa_sl, I_Clbk, I_Catot, J_serca, I_Ca_junc, I_Ca_sl,0,0,0,Jrel_ICaLdep, I_tof, I_tos, NaN, NaN, Ta, ...
...
CaMK_Phos_ss_ICaL =  CaMK_active / (CaMK_active + K_Phos_CaMK);
CaMK_Phos_ss_RyR =  CaMK_active / (CaMK_active + 1);
CaMK_Phos_ss_PLB =  CaMK_active / (CaMK_active + 10);

d_camk_f_ICaL = (CaMK_Phos_ss_ICaL - camk_f_ICaL) / tau_cal;
d_camk_f_RyR = (CaMK_Phos_ss_RyR - camk_f_RyR)  / tau_ryr;
d_camk_f_PLB = (CaMK_Phos_ss_PLB - camk_f_PLB)  / tau_plb;
...
GNa_P= GNa * 1.25; %11.7802*1.7; %BetaAdrenergic;
...
function [ICaL_junc,ICaNa_junc,ICaK_junc,ICaL_sl,ICaNa_sl,ICaK_sl,dd,dff,dfs,dfcaf,dfcas,...
    djca,dnca, dnca_i, dffp,dfcafp, dd_P,dff_P,dfs_P,dfcaf_P,dfcas_P,dfBPf,dfcaBPf, d_ical_pureCDI_junc, d_ical_pureCDI_sl, PhiCaL_ss, PhiCaL_i, gammaCaoMyo, gammaCaiMyo] = getICaL_ORd2011_jt(v, d,ff,fs,fcaf,fcas,jca,nca, nca_i,ffp,fcafp,...
...
PhiCaK_ss =  1.0*vffrt*(gamma_ki*ions_k_i*exp(1.0*vfrt)-gamma_ko*ko)/(exp(1.0*vfrt)-1.0);
...
function [ICaL_junc,ICaNa_junc,ICaK_junc,ICaL_sl,ICaNa_sl,ICaK_sl,dd,dff,dfs,dfcaf,dfcas,...
    djca,dnca, dnca_i, dffp,dfcafp, dd_P,dff_P,dfs_P,dfcaf_P,dfcas_P,dfBPf,dfcaBPf, d_ical_pureCDI_junc, d_ical_pureCDI_sl, PhiCaL_ss, PhiCaL_i, gammaCaoMyo, gammaCaiMyo] = getICaL_ORd2011_noPKA_jt(v, d,ff,fs,fcaf,fcas,jca,nca, nca_i,ffp,fcafp,...
    d_P,ff_P,fs_P,fcaf_P,fcas_P,fBPf,fcaBPf,...
    ical_pureCDI_junc, ical_pureCDI_sl, fICaLp, fICaLP, ions_ca_i, ions_ca_sl, ions_ca_junc, ions_ca_o, ions_na_i, ions_na_sl, ions_na_o, ions_k_i, ko, ions_cl_i, clo, celltype, ICaL_fractionSS, ICaL_PCaMultiplier, extraParams)
...
%% Calculating "pure" CDI
sigmoidTransition = 1 - 1/(1 + (1.86532 * ions_ca_junc/0.032)^(0.99613));
tauTransition = 1.09670 + (1-sigmoidTransition)*141.42990;
...
% and for nondyadic
sigmoidTransition2 = 1 - 1/(1 + (1.86532 * ions_ca_sl/0.032)^(0.99613));
tauTransition2 = 1.09670 + (1-sigmoidTransition2)*141.42990;
...
if cellType == 1 % epi. mid is 2
    GtoSlow=0.02036 * Itos_Multiplier; %epi
    GtoFast=0.29856 * Itof_Multiplier; %epi0.88
elseif cellType == 2 % epi. mid is 2
    GtoSlow= 0.04632 * Itos_Multiplier; %endo
    GtoFast=0.14928 * Itof_Multiplier;%endo; %epi - 0.875 scaling based on https://www.ahajournals.org/doi/pdf/10.1161/01.RES.72.3.671
...
function [IKr, dc0, dc1, dc2, do, di ] = getIKr_ToRORd_MM(V,c0,c1, c2, o, i,...
...
if celltype==1
    GKr=GKr*1.25; % changed compared to baseline
elseif celltype==2
...
kPKA_Iks = fIKs_PKA;
gks_factor_SA = 2.97002 * IKs_Multiplier;
...
caks_sl = ions_ca_sl; % normal simulation
...
I_ks_junc = Fjunc*gks_factor_SA*gks_junc*iks_xs_junc^2*(membrane_v-eks);
I_ks_sl = Fsl*gks_factor_SA*gks_sl*iks_xs_sl^2*(membrane_v-eks);
...
function [ INaCa_i, INaCa_ss] = getINaCa_ORd2011(v,F,R,T, nass, nasl, nai, nao, cass, casl, cai, cao, celltype, INaCa_Multiplier, INaCa_fractionSS)
...
E4=x4/(x1+x2+x3+x4);
KmCaAct=150e-6;
allo=1.0/(1.0+(KmCaAct/casl)^2);
% allo = 1 - 1/(1+((casl*1e6)/107)^2.57);
zna=1.0;
...
ks = 26.6 * Jrel_Multiplier;                 % [1/ms]
koCa = 23.87221;               % [mM^-2 1/ms]   %default 10   modified 20
kom = 0.16219;              % [1/ms]
kiCa = 0.39871;              % [1/mM/ms]
kim = 0.04311;             % [1/ms]
ec50SR = 0.75385;           % [mM]
...
ecCaI = 0.001;
steepnessCaI = 5.93447; % change compared to 5.7.4.5.4 was reflected here
minCaI = 0.93249;
maxCaI = 30.13294;
...
Qpow = (Temp-310)/10;
Q10SRCaP = 2.6;          % [none]
Vmax_SRCaP = 0.00543 * Jup_Multiplier;  % [mM/msec] (286 umol/L cytosol/sec)

Kmr = 2.31442;               % [mM]L cytosol
hillSRCaP =  1.02809;       % [mM]
Kmf = 0.30672e-03;          % [mM] default
...
Kmf_Phospho = Kmf * 0.5; %; % Similar percentage effect as in Heijman 2011 % CHANGED JAKUB

% Direct Ca-based acceleration
Km_SERCA_Ca = 0.4; %0.03 in Heijman 2011; affinity for direct Vmax modulation by CaMKII
...
PKAXBacceleration = 1 + 0.5 * fMyBPC_PKA;
